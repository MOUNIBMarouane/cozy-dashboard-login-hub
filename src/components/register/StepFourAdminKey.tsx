...
const StepFourAdminKey = () => {
  ...
  // Show admin input only if checked or previously set
  const [showAdminField, setShowAdminField] = useState<boolean>(!!formData.adminSecretKey);

  useEffect(() => {
    if (!showAdminField) {
      // If user disables admin checkbox, clear admin key field and immediately move to next step
      setFormData({ adminSecretKey: '' });
      // On first render do not auto-move; only on change
